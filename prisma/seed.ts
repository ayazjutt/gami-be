...
async function main() {
  ...
  // Only Networks and Settings are seeded below.
...
for (const s of settings) {
  ...
  // Alert threshold settings requested
  const alertThresholdSettings = [
    { key: 'PT-3M Mispricing Threshold', value: null, numValue: '0.02', notes: 'Percent (decimal). 2% mispricing alert for PT-3M' },
    { key: 'YT-3M Harvest Threshold',    value: null, numValue: '500',  notes: 'USD threshold to trigger YT-3M harvest' },
    { key: 'Liquidity Crisis Threshold',  value: null, numValue: '-0.5', notes: 'Liquidity drop threshold (decimal). -50%' },
    { key: 'Gas Spike Threshold',         value: null, numValue: '100',  notes: 'Gwei threshold for gas spike alerts' },
    { key: 'Maturity Alert Threshold',    value: null, numValue: '30',   notes: 'Days before maturity to raise alert' },
  ];
  for (const s of alertThresholdSettings) {
    await prisma.setting.upsert({
      where: { key: s.key },
      update: { value: s.value, numValue: s.numValue, notes: s.notes },
      create: s,
    });
  }
